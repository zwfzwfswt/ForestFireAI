...
import { Comment, cloneVNode, createTextVNode, createVNode, defineComponent, isVNode, renderSlot } from "vue";
				...
				if (spacer) {
					...
					extractedChildren = extractedChildren.reduce((acc, child, idx) => {
						...
						if (idx !== len) children.push(createVNode("span", {
							...
							key: idx
						}, [isVNode(spacer) ? cloneVNode(spacer) : createTextVNode(spacer, 1)], 4));
						return children;
					}, []);
				}
