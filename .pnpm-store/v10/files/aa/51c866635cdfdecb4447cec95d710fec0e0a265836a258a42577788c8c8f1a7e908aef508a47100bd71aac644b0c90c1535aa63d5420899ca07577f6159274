...
function createContext(options = {}, root = process.cwd()) {
	...
	biomelintrc.enabled = biomelintrc.enabled === void 0 ? false : biomelintrc.enabled;
...
${dts}`.trim()}\n`;
	...
	async function writeConfigFiles() {
		...
		if (dumpUnimportItems) promises.push(unimport.getImports().then((items) => {
	...
	const normalizedDirMatchers = (dirs?.length ? dirs.flatMap((dir) => normalizeScanDirs([dir], {
		...dirsScanOptions,
		cwd: root
	})) : []).map((dir) => pm(dir.glob));
	return {
	...
	return {
		...
		vite: {
			...
			async handleHotUpdate({ file }) {
				...
				if (ctx.normalizedDirMatchers.some((match) => match(normalizedFilePath))) await ctx.scanDirs();
			},
