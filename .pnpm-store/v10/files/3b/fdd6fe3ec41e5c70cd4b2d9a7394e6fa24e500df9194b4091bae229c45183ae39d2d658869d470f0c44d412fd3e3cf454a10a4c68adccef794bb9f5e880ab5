...
function createFilter(include, exclude) {
	...
	const match = (id) => {
		if (excludePattern.some((p) => id.match(p))) return;
		return includePattern.find((item) => {
			const pattern = item instanceof RegExp || typeof item === "string" ? item : item.pattern;
	...
	return {
		...
		resolveResolver: (id) => {
			...
			if (matched && !(matched instanceof RegExp) && typeof matched !== "string") return matched.resolver;
		}
	};
}
function transformerAttributifyJsx(options = {}) {
	...
	const filter = createFilter(options.include || [/\.[jt]sx$/, /\.mdx$/], options.exclude || []);
	return {
