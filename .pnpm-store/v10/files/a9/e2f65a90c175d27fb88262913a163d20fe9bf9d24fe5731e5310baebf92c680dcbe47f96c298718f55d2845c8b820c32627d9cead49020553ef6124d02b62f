...
const usePanelDateRange = (props, emit, leftDate, rightDate) => {
	...
	const disabledDate = (0, vue.toRef)((0, vue.inject)(require_constants.PICKER_BASE_INJECTION_KEY).props, "disabledDate");
	...
	async function handlePick(mode, pickerType, value) {
		...
		if (mode === "year") startDate.value = require_utils.getValidDateOfYear(startDate.value.year(value), lang.value, disabledDate.value);
