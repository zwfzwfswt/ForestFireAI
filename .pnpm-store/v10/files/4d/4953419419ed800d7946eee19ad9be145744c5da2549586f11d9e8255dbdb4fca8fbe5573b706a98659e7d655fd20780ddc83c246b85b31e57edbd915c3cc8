(function(exports, _vueuse_shared, vue) {
	...
	*/
	function templateRef(key, initialValue = null) {
		...
		(0, _vueuse_shared.tryOnMounted)(_trigger);
	...
	*/
	...
	function getDefaultScheduler$8(options) {
		if ("interval" in options || "immediate" in options) {
			...
			return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, interval, { immediate });
		}
		return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 1e3, { immediate: false });
	}
	/**
	...
	*/
	...
	function getDefaultScheduler$7(options) {
		if ("interval" in options || "immediate" in options) {
			...
			return interval === "requestAnimationFrame" ? (cb) => useRafFn(cb, { immediate }) : (cb) => (0, _vueuse_shared.useIntervalFn)(cb, interval, { immediate });
		}
		return useRafFn;
	}
	/**
	...
	*/
	function useIntersectionObserver(target, callback, options = {}) {
		...
		const targets = (0, vue.computed)(() => {
			return (0, _vueuse_shared.toArray)((0, vue.toValue)(target)).map(unrefElement).filter(_vueuse_shared.notNullish);
		});
	...
	*/
	function useInfiniteScroll(element, onLoadMore, options = {}) {
		...
		(0, _vueuse_shared.tryOnUnmounted)((0, vue.watch)(() => [
	...
	*/
	...
	function getDefaultScheduler$6(options) {
		if ("interval" in options || "immediate" in options || "immediateCallback" in options) {
			...
			return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, interval, {
				immediate,
				immediateCallback
			});
		}
		return _vueuse_shared.useIntervalFn;
	}
	/**
	...
	*/
	...
	function getDefaultScheduler$5(options) {
		if ("interval" in options || "immediate" in options) {
			const { interval = "requestAnimationFrame", immediate = true } = options;
			return interval === "requestAnimationFrame" ? (fn) => useRafFn(fn, { immediate }) : (fn) => (0, _vueuse_shared.useIntervalFn)(fn, interval, options);
		}
		return useRafFn;
	}
	/**
	...
	*/
	...
	function getDefaultScheduler$4(options) {
		if ("updateInterval" in options) {
			...
			return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, updateInterval);
		}
		return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 3e4);
	}
	/**
	...
	*/
	...
	function getDefaultScheduler$1(options = {}) {
		...
		return (fn) => (0, _vueuse_shared.useIntervalFn)(fn, interval, {
			immediate: false,
	...
	*/
	...
	function getDefaultScheduler(options) {
		if ("interval" in options) {
			...
			return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, interval, { immediate: false });
		}
		return (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 1e3, { immediate: false });
	}
	/**
