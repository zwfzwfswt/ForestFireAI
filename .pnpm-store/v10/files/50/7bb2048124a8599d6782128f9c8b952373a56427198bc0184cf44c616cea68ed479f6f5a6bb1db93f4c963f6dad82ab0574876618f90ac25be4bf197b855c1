...
const usePanelDateRange = (props, emit, leftDate, rightDate) => {
	...
	const disabledDate = toRef(inject(PICKER_BASE_INJECTION_KEY).props, "disabledDate");
	...
	async function handlePick(mode, pickerType, value) {
		...
		if (mode === "year") startDate.value = getValidDateOfYear(startDate.value.year(value), lang.value, disabledDate.value);
