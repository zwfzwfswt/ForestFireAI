...
`src/stores/uavSimulator.ts` 持有一个非响应式调度器，Pinia 仅暴露可序列化运行状态。地图与资产页面使用同一 owner 集合，挂载/激活幂等获取，停用/卸载释放；最后一个相关页面离开后清除 timer，再进入时恢复此前运行状态。全部暂停期间保留同一个时钟用于离线判定；无独立逐机 timer。退出登录或切换租户沿用 UAV 会话重置并停止模拟器。刷新不持久化任何模拟数据。
