...
import type { FireEvent } from "../../../../fire/types";
...
export function firePopup(event: FireEvent) {
  return (
    [
      "火情事件 · Mock",
      event.code,
      event.title,
      `等级：${fireLevelConfig[event.level].label}`,
      `状态：${fireStatusConfig[event.status].label}`,
      `发现：${fireTime(event.detectedAt)}`,
      `来源：${fireSourceConfig[event.source].label}`,
    ]
      .map(escapeHtml)
      .join("<br>") + '<br><button type="button" class="fire-popup-detail">查看详情</button>'
  );
}
export function useFireMapLayer(isDrawing: () => boolean) {
  ...
  function sync(L: typeof Leaflet) {
    ...
    for (const event of store.events) {
      const options = {
        className: "ff-fire-symbol",
        html: fireSymbolHtml(event.status, event.level),
      ...
      if (!marker) {
        ...
        marker.on("popupopen", () => {
          const button = marker!
            .getPopup()
            ?.getElement()
            ?.querySelector<HTMLButtonElement>(".fire-popup-detail");
      ...
      marker.options.title = `${event.code} · ${event.title} · ${fireStatusConfig[event.status].label} · ${fireLevelConfig[event.level].label} · Mock`;
      const element = marker.getElement();
      if (element) element.title = marker.options.title;
      // 更新内容前关闭 Popup，确保详情按钮绑定对应最新 DOM。
      marker.closePopup().unbindPopup();
      if (!isDrawing()) marker.bindPopup(firePopup(event), { pane, autoPan: false });
    }
  }
  ...
  function attach(map: LeafletMap, L: typeof Leaflet, registry: MapLayerRegistry) {
    ...
    const update = watch(
      ...
      () => {
        sync(L);
        registry.setFeatureCount("FireEventLayer", markers.size);
      },
      { flush: "sync" }
    );
    ...
    const locate = watch(
      ...
      (request) => {
        if (!request) return;
        const event = store.events.find((item) => item.id === request.id);
        if (!event) return;
        registry.show("FireEventLayer");
        if (registry.getState("FireEventLayer")?.opacity === 0)
          registry.setOpacity("FireEventLayer", 1);
        map.flyTo(
